<template>
    ...
    <div class="form form_category_in">
        ...
        <div class="form-item_input">
          ...
          <div class="form-group ms-small">
            <base-input
              ...
              :type="'text'"
              :maxLength="25"
              :messageValid="t('validate.empty', { field: t('module.inventory.categoryCode') })"
              :label="$t('common.id')" 
              v-model="category.categoryCode"
              :class="{ 'is-valid': isValid && category.categoryCode == '' }"
              ref="inputFocus"
            ></base-input>
          </div>
          <div class="form-group ms-big">
            <base-input
              ...
              :type="'text'"
              :maxLength="255"
              :messageValid="t('validate.empty', { field: t('module.inventory.categoryName') })"
              :label="$t('common.name')"
              v-model="category.categoryName"
              :class="{ 'is-valid': isValid && category.categoryName == '' }"
            ></base-input>
          </div>
        </div>
...
import { ActionTable, Category, ENotificationType, Grid, ModuleName, ServiceResponse } from '@/core/public_api';
...
const categoryComparison = ref<Category>(new Category());
...
onBeforeMount(() => {
  if(props.Base.StateForm === ActionTable.Edit || props.Base.StateForm === ActionTable.Replication){
    category.value = {...props.Base.RecordEdit};
    categoryComparison.value = {...props.Base.RecordEdit};
  }
})
...
async function handleSaveData(closeModal: any) {
  try {
    ...
    if (messValid.length > 0) {
      isValid.value = true;
      unListenEvent();
      props.Base.showNotificationError(messValid, ()=> { props.Base.focusInputError('form_category_in'), listenEvent() });
    }
    else {
      ...
      if (closeModal === true && errorApi.value === false) {
        ...
        categoryComparison.value = new Category();
...
function validateInput() {
  return [
    /** Validate code */ 
    category.value.categoryCode.trim() === "" ? t('validate.empty', { field: t('module.inventory.categoryCode') }) : null,
    /** Validate name */
    category.value.categoryName.trim() === "" ? t('validate.empty', { field: t('module.inventory.categoryName') }) : null,
...
const handleCloseModal = () => {
  try {
    if (JSON.stringify(category.value) != JSON.stringify(categoryComparison.value)) {
      unListenEvent();
