<template>
    ...
    <div class="form form_trademark_in">
      ...
      <div class="form-container">
        ...
        <div class="form-item_input">
          ...
          <div class="form-group ms-small">
            <base-input
              ...
              :type="'text'"
              :maxLength="25"
              :messageValid="t('validate.empty', { field: t('module.inventory.trademarkCode') })"
              :label="$t('common.id')" 
              v-model="trademark.trademarkCode"
              :class="{ 'is-valid': isValid && trademark.trademarkCode == '' }"
              ref="inputFocus"
            ></base-input>
          </div>
          <div class="form-group ms-big">
            <base-input
              ...
              :type="'text'"
              :maxLength="255"
              :messageValid="t('validate.empty', { field: t('module.inventory.trademarkName') })"
              :label="$t('common.name')"
              v-model="trademark.trademarkName"
              :class="{ 'is-valid': isValid && trademark.trademarkName == '' }"
            ></base-input>
          </div>
        </div>
        ...
        <div class="form-group ms-big">
          <base-image v-model="trademark.avatar"></base-image>
        </div>
      </div>
...
import { ActionTable, Trademark, ENotificationType, Grid, ModuleName, ServiceResponse } from '@/core/public_api';
 ...
 * Khắc Tiềm - 08.03.2023
 */
const trademark = ref<Trademark>(new Trademark());
const trademarkComparison = ref<Trademark>(new Trademark());
...
onBeforeMount(() => {
  if(props.Base.StateForm === ActionTable.Edit || props.Base.StateForm === ActionTable.Replication){
    trademark.value = {...props.Base.RecordEdit};
    trademarkComparison.value = {...props.Base.RecordEdit};
  }
})
...
async function handleSaveData(closeModal: any) {
  try {
    ...
    if (messValid.length > 0) {
      isValid.value = true;
      unListenEvent();
      props.Base.showNotificationError(messValid, ()=> { props.Base.focusInputError('form_trademark_in'), listenEvent() });
    }
    else {
      ...
      if (closeModal === true && errorApi.value === false) {
        ...
        trademarkComparison.value = new Trademark();
...
function validateInput() {
  return [
    /** Validate code */ 
    trademark.value.trademarkCode.trim() === "" ? t('validate.empty', { field: t('module.inventory.trademarkCode') }) : null,
    /** Validate name */
    trademark.value.trademarkName.trim() === "" ? t('validate.empty', { field: t('module.inventory.trademarkName') }) : null,
...
const handleCloseModal = () => {
  try {
    if (JSON.stringify(trademark.value) != JSON.stringify(trademarkComparison.value)) {
      unListenEvent();
