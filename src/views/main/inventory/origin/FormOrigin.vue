        ...
        <div class="form-item_input">
          ...
          <div class="form-group ms-small">
            <base-input
              ...
              :type="'text'"
              :maxLength="25"
              :messageValid="t('validate.empty', { field: t('module.inventory.originCode') })"
              :label="$t('common.id')" 
          ...
          <div class="form-group ms-big">
            <base-input
              ...
              :type="'text'"
              :maxLength="255"
              :messageValid="t('validate.empty', { field: t('module.inventory.originName') })"
              :label="$t('common.name')"
...
import { ActionTable, Origin, ENotificationType, Grid, ModuleName, ServiceResponse } from '@/core/public_api';
...
async function handleSaveData(closeModal: any) {
  try {
    ...
    if (messValid.length > 0) {
      isValid.value = true;
      unListenEvent();
      props.Base.showNotificationError(messValid, ()=> { props.Base.focusInputError('form_origin_in'), listenEvent() });
    }
    else {
...
const callApiForm = async (api: any, stateForm: string = '') => {
  errorApi.value = true;
  await props.Base.apiService.callApi(api, origin.value ,async (response: any) => { 
    if (props.Base.StateForm === ActionTable.Add || props.Base.StateForm === ActionTable.Replication){
      props.Base.store.dispatch(`${props.Base.Module}/addRecordAction`, { ...origin.value, originID: response });
    }
    else{
      props.Base.store.dispatch(`${props.Base.Module}/editRecordAction`, { ...origin.value, originID: response });
      props.Base.setStateForm(ActionTable.Add);
    }
...
function validateInput() {
  return [
    /** Validate code */ 
    origin.value.originCode.trim() === "" ? t('validate.empty', { field: t('module.inventory.originCode') }) : null,
    /** Validate name */
    origin.value.originName.trim() === "" ? t('validate.empty', { field: t('module.inventory.originName') }) : null,
...
const handleCloseModal = () => {
  try {
    if (JSON.stringify(origin.value) != JSON.stringify(originComparison.value)) {
      unListenEvent();
